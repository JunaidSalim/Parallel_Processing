...
void* parallelBitonicSort(void* arg) {
    ...
    if (cnt > 1) {
        ...
        if (cnt > THRESHOLD) {
            pthread_t tid;
            SortArgs leftArgs = {args->arr, low, k, 1};
            pthread_create(&tid, NULL, parallelBitonicSort, &leftArgs);
            
            SortArgs rightArgs = {args->arr, low + k, k, 0};
            parallelBitonicSort(&rightArgs);
            
            pthread_join(tid, NULL);
            bitonicMerge(args->arr, low, cnt, dir);
        } else {
            bitonicSort(args->arr, low, cnt, dir);
        }
    }
    return NULL;
}
