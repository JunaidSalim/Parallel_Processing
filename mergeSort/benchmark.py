...
def runBenchmarks():
    ...
    for arraySize in arraySizes:
        ...
        for _ in range(numRuns):
            timeSeq = runSort('sequential_merge', arraySize)
            if timeSeq: sizeResults['sequential'].append(timeSeq)
            
            timePthread = runSort('pthread_merge', arraySize)
            if timePthread: sizeResults['pthread'].append(timePthread)
            
            timeOpenmp = runSort('openmp_merge', arraySize)
            if timeOpenmp: sizeResults['openmp'].append(timeOpenmp)
            
            timeMpi = runSort('mpi_merge', arraySize, 4)
...
def plotResults(arraySizes, results):
    ...
    plt.title('Merge Sort Algorithm Performance Comparison')
    plt.grid(True)
    plt.legend()
    ...
    plotFilename = f'graphs/merge_benchmark.png'
    plt.savefig(plotFilename)
    plt.close()

if __name__ == '__main__':
    print("Starting merge sort benchmarks...")
    arraySizes, results = runBenchmarks()
    plotResults(arraySizes, results)
    print("\nBenchmarking complete. Results have been saved to graphs/merge_benchmark.png")