...
def runBenchmarks():
    ...
    for arraySize in arraySizes:
        ...
        for _ in range(numRuns):
            timeSeq = runSort('sequential_radix', arraySize)
            if timeSeq: sizeResults['sequential'].append(timeSeq)
            
            timePthread = runSort('pthread_radix', arraySize)
            if timePthread: sizeResults['pthread'].append(timePthread)
            
            timeOpenmp = runSort('openmp_radix', arraySize)
            if timeOpenmp: sizeResults['openmp'].append(timeOpenmp)
            
            timeMpi = runSort('mpi_radix', arraySize, 4)
...
def plotResults(arraySizes, results):
    ...
    plt.title('Radix Sort Algorithm Performance Comparison')
    plt.grid(True)
    plt.legend()
    ...
    plotFilename = f'graphs/radix_benchmark.png'
    plt.savefig(plotFilename)
    plt.close()

if __name__ == '__main__':
    print("Starting radix sort benchmarks...")
    arraySizes, results = runBenchmarks()
    plotResults(arraySizes, results)
    print("\nBenchmarking complete. Results have been saved to graphs/radix_benchmark.png")